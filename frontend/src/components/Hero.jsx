...
import HeroTodoTemplate from "./HeroTodoTemplate";
function Hero(){
    return(
        <>
            <div className="relative min-h-screen w-full">
                ...
                <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-5xl flex-col items-center justify-center px-6 pt-20 text-center">
                    ...
                    <div className="mt-8 flex flex-col items-center gap-4 sm:flex-row">
                        <GlowButton text="Start for free" to="/signup"/>
                        <Link className="rounded-md border border-(--border-h) bg-[rgba(255,255,255,0.08)] px-5 py-2 text-sm font-semibold text-(--text) shadow-[0_10px_30px_rgba(15,23,42,0.12)] backdrop-blur-sm transition hover:-translate-y-0.5 hover:border-(--bg-glow) hover:bg-[rgba(127,30,212,0.12)] hover:text-(--bg-glow) focus:outline-none focus:ring-2 focus:ring-(--bg-glow) focus:ring-offset-2 focus:ring-offset-transparent" to="/signin">Sign in</Link>
                    </div>
                    <HeroTodoTemplate/>
                </div>
            </div>
